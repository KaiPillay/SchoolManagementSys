...
{
    public partial class TeacherViewAnnouncements : System.Web.UI.Page
    {
        ...
        private void BindAnnouncements()
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                try
                {
                    ...
                    // Query to fetch announcements for the teacher
                    string query = @"SELECT a.* 
                                    FROM Announcements a
                                    JOIN AnnouncementTargets t ON a.AnnouncementID = t.AnnouncementID
                                    WHERE t.TargetType = 'AllStaff'
                                       OR (t.TargetType = 'Class' AND t.ClassID IN (SELECT ClassID FROM teachersubject WHERE TeacherID = @TeacherID))
